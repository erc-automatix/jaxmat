...
def rotations():
    ...
    return random_rotations(key, shape=(3,))
...
def test_eig33_reconstruction_distinct(rotations):
    ...
    eigvals_b, dyads_b = jax.jit(jax.vmap(eig33))(A_batch)
    A_rec = jnp.einsum("bi,bijk->bjk", eigvals_b, dyads_b)
    assert jnp.allclose(A_batch, A_rec, atol=1e-5)
...
def test_eig33_reconstruction_two_equal(rotations, eps):
    ...
    A_batch = jax.vmap(_build, in_axes=(None, 0))(diag, rotations[:3])
    eigvals_b, dyads_b = jax.jit(jax.vmap(eig33))(A_batch)
    A_rec = jnp.einsum("bi,bijk->bjk", eigvals_b, dyads_b)
    assert jnp.allclose(A_batch, A_rec, atol=1e-5)
...
def test_eig33_gradient_finite():
    _jac = jax.jit(jax.jacobian(lambda A: eig33(A)[0]))
    g = _jac(_spd(jax.random.PRNGKey(3)))
    assert jnp.all(jnp.isfinite(g))


def test_eig33_gradient_finite_repeated():
    _jac = jax.jit(jax.jacobian(lambda A: eig33(A)[0]))
    g = _jac(2.0 * jnp.eye(3))
    assert jnp.all(jnp.isfinite(g))
...
def test_eig33_vmap(rotations):
    diag = jnp.array([1.0, 2.0, 3.0])
    A_batch = jax.vmap(_build, in_axes=(None, 0))(diag, rotations)
    eigvals_batch, _ = jax.jit(jax.vmap(eig33))(A_batch)
...
def test_isotropic_function_matches_scipy(scalar_fun, scipy_fun, rotations):
    ...
    for R in rotations[:4]:
        ...
        assert jnp.allclose(
            ...
            jnp.array(jnp.real(scipy_fun(jnp.array(A)))),
            atol=1e-8,
        )
...
def test_sqrtm_round_trip(rotations):
    ...
    A_batch = jax.vmap(_build, in_axes=(None, 0))(diag, rotations[:4])
    U_batch = jax.vmap(sqrtm)(A_batch)
    assert jnp.allclose(jnp.einsum("bij,bjk->bik", U_batch, U_batch), A_batch, atol=1e-5)
...
def test_inv_sqrtm_round_trip(rotations):
    ...
    diag = jnp.array([1.0, 2.0, 4.0])
    A_batch = jax.vmap(_build, in_axes=(None, 0))(diag, rotations[:4])
    Anh_batch = jax.vmap(inv_sqrtm)(A_batch)
    prod = jnp.einsum("bij,bjk,bkl->bil", Anh_batch, A_batch, Anh_batch)
    assert jnp.allclose(prod, jnp.eye(3), atol=1e-5)
...
def test_expm_logm_round_trip(rotations):
    diag = jnp.array([1.0, 2.0, 3.0])
    A_batch = jax.vmap(_build, in_axes=(None, 0))(diag, rotations[:4])
    rt = jax.vmap(lambda A: expm(logm(A)))(A_batch)
    assert jnp.allclose(rt, A_batch, atol=1e-5)
