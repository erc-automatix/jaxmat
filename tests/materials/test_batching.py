...
elastoplastic = jm.vonMisesIsotropicHardening(elasticity=elasticity, yield_stress=hardening)
...
def test_material_batching(material):
    def eval_stress(material, eps):
        ...
        sig, new_state = material.constitutive_update(eps, state, 0.0)
        return sig
    ...
    assert jnp.allclose(sig_batched.array, jnp.broadcast_to(sig.array, sig_batched.shape))
