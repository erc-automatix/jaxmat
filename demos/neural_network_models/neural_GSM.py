...
#
# In this demo, we implement a Generalized Standard Material (GSM) model of viscoelasticity, in
# which part of the model is represented by a neural network. This approach follows the general idea
# proposed by {cite:p}`flaschel2025convex`, where neural convex potentials are embedded in
# thermodynamically consistent constitutive models.
#
# The goal is to demonstrate how a data-driven, physics-constrained constitutive model can be
# trained to reproduce a reference viscoelastic response generated by a classical model, while
# keeping full consistency with the framework of generalized standard materials.
#
...
# ```
#
# To simplify the exposition, we focus exclusively on a viscoelastic material. The reference
# (“ground truth”) data is produced using a Generalized Maxwell model with 6 branches, with randomly
# generated viscoelastic properties.
#
# The training objective is to identify a neural GSM model with a reduced number of internal state
# variables, denoted $N_\text{var}$, while retaining the correct stress relaxation behavior. of
# internal state variables. The long-term elastic response is assumed to be known.
#
# ## Theoretical background
#
# ### Generalized Standard Materials
#
# A generalized standard material {cite:p}`halphen1975materiaux` is defined by a free energy
# potential $\Psi(\beps,\balpha)$ and a dissipation potential $\Phi(\dot\balpha)$, from which all
# constitutive relations are derived in a thermodynamically consistent way. The internal state
# variable evolution over a time-step is obtained by minimizing a so-called *incremental potential*
# {cite:p}`ortiz1999variational`:
#
# $$\min_{\Delta \balpha} J(\Delta\balpha ;\beps_{n+1},\balpha_n) = \min_{\Delta \balpha}
# \Psi(\beps_{n+1}, \balpha_n + \Delta \balpha)
...
# - the viscous effects are governed by a neural dissipation potential represented by an Input
#   Convex Neural Network (ICNN) {cite:p}`amos2017icnn`.
#
# The constitutive potentials are written as:
# - free energy:
#
# $$
# \Psi(\beps,\balpha_i) = \dfrac{1}{2}\beps:\CC_0:\beps +
# \sum_{i=1}^{N_\text{var}}\dfrac{1}{2}(\beps-\balpha_i):\CC_i:(\beps-\balpha_i)
# $$
#
...
# $$
# \Phi(\dot{\balpha}_i) =
# \mathcal{N}_\text{ICNN}(\dot{\balpha}_1,\ldots,\dot{\balpha}_{N_\textrm{var}})
# $$
#
# Unlike the classical Maxwell model where each branch dissipates independently, the neural
# potential couples all internal variables through a single convex function $\Phi$. This allows for
# nonlinear and non-separable viscous interactions while ensuring that the model remains
# thermodynamically admissible.
#
# ```{note}
# The ground truth generalized Maxwell model with $N$ branches would be represented by the following
# potentials where the $\balpha_i$ represent the viscous strains $\beps^\text{v}_i$ in each branch:
# - free energy:
#
# $$
# \Psi(\beps,\beps_i^\text{v}) = \dfrac{1}{2}\beps:\CC_0:\beps +
# \sum_{i=1}^{N}\dfrac{1}{2}(\beps-\beps_i^\text{v}):\CC_i:(\beps-\beps_i^\text{v})
# $$
#
# - dissipation potential:
#
# $$
# \Phi(\dot{\beps}_i^\text{v}) = \sum_{i=1}^N \dfrac{1}{2}\eta_i
...
# %% [markdown]
# To train the model, we consider a stress relaxation (recovery) test under constant shear strain:
...
# %% [markdown]
# We define a general-purpose function `compute_evolution` that integrates the constitutive
# equations over time for a given strain path. Starting from the initial state, the model is updated
...
#
# - Free energy potential, defines the recoverable elastic energy stored in both the equilibrium and
#   viscous branches.
# - Dissipation potential, modeled by an Input Convex Neural Network (ICNN) to ensure convexity with
#   respect to strain rates.
...
# %% [markdown]
# The dissipation potential is represented by an Input-Convex Neural Network (ICNN) which guarantees
# convexity through positive weights and nonnegative increasing activation functions.
#
# Inspired by the Physics-Augmented Neural networks (PANN) approach for elasticity, we choose here
# as inputs of the network the second invariants $I_2(\balpha_i)$ of the state variables. We also
# add a normalization term so that the thermodynamic force is zero for $\balpha_i=0$, modifying the
...
# %% [markdown]
# We build a list of candidate neural GSM models for  $N_\text{var}=1,2,3$. Each model shares the
# same known long-term stiffness, while the viscous stiffness values and neural potential parameters
# are initialized randomly. We select here only 8 neurons in a single hidden layer.
#
# ```{important}
# Since the structure of internal state variables is not known in advance,
# `jaxmat.materials.GeneralizedStandardMaterial` is an abstract class which must implement a
# concrete `make_internal_state` method.
# ```
#
...
# ### Initialization
#
# Before training, we compute the ground truth response under the prescribed strain history obtained
# from the 6-branch Generalized Maxwell model. We generate synthetic white noise to this data.
# Finally, we compute the predicted stress response of the randomly initialized neural GSM (for
...
# ### Loss function
#
# The training process aims to minimize the discrepancy between the predicted and target stress
# histories. We use the mean-squared error (MSE) loss function on the whole stress tensors:
#
# $$
# \mathcal{L}(\btheta) = \dfrac{1}{M}\sum_{k=1}^M \|\hat\bsig^{(k)}(\btheta) -
# \bsig^\text{data,(k)}\|^2
# $$
# where $\hat\bsig^{(k)}(\btheta)$ denotes the predicted stress tensor by the neural GSM at time
# step $k$ and $ \bsig^\text{data,(k)}$ the corresponding target value.
# As the stress evolution depends recursively on the internal variables, the loss is
# history-dependent.
...
#
# We employ an `optax` optimizer based on Adam with parameter-block RMS normalization, which
# rescales gradients according to their variance across model parameters. This helps stabilize
# training in the presence of quantities with different physical magnitudes.
#
...
#
# During training, the equilibrium elastic model is kept frozen (not trainable), the viscous
# stiffness values and ICNN parameters are updated to minimize the loss.
#
# The backward-mode automatic differentiation (`jac="bwd"`) option is used for efficiency, as we are
# minimizing a scalar loss function. At the end of training, the trained parameters are combined
# with the static part to reconstruct the full model.
#
# For each value of $N_\text{var}$, we plot the predicted relaxation curve against the ground truth
# data.
#
# The model with a single viscoelastic strain fails in reproducing the whole relaxation curve,
# despite being able to represent more than a single relaxation term since the viscosity dissipation
# potentially is not necessarily quadratic. Increasing $N_\text{var}$ to 2 and 3 clearly improves
# the results. The trained neural GSM reproduces the full multi-timescale relaxation curve almost
# perfectly.
#
